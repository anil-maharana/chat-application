...
const ChatArea = ({
  ...
  const [selectedRoomFromState, setSelectedRoomFromState] =
    useState(selectedRoom);
  ...
  const handleMessageSubmit = (newMessage) => {
    ...
    socketContext.emit(
      "send-message",
      { message: newMessage, author: user, room: selectedRoomFromState },
      async () => {
        ...
        const _msg = {
          ...
          timestamp: new Date(data.createdOn).toLocaleString("en-US", {
            hour: "numeric",
            minute: "numeric",
            hour12: true,
          }),
  ...
  useEffect(() => {
    ...
    socket.on("new-message-notification", ({ roomID, message }) => {
      ...
      if (roomID !== selectedRoomFromState._id)
        newNotification(message, roomID);
    });
  }, [socketContext]);
  ...
  const fetchMessages = async (roomId) => {
    ...
    const res = await axios.get(
      `${configData.SERVER_URL}/api/messages/${roomId}`
    );
    const _msg = res.data.map((m) => {
      return {
        ...
        timestamp: new Date(m.createdOn).toLocaleString("en-US", {
          hour: "numeric",
          minute: "numeric",
          hour12: true,
        }),
  ...
  const saveMessage = async (message) => {
    const res = await axios.post(`${configData.SERVER_URL}/api/messages`, {
